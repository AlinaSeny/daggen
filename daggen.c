...
int main(int argc, char **argv)
{
  ...
  /* putting header */
  fprintf(stderr, "// DAG automatically generated by daggen at %s// ",
      ctime(&now));
  for (i=0;i<argc;i++) {
  ...
  if (global.conn) {
    ...
    while (1) {
      ...
      if (connected) {
        fprintf(stderr, "Dag connected after %d attemts.\n", nbtry);
        break;
      }
      freeDAG(dag);
      nbtry++;
      if (nbtry == 10) {
        fprintf(stderr, "Dag disconnected after 10 attemts. You can try again @_@. Good luck!\n");
        exit(0);
      }
    }
  } else {
    ...
    for (j=0; j<dag->nb_tasks_per_level[i]; j++) {
      if (!dag->levels[i][j]->checked) {
        ...
        if (DFS(dag->levels[i][j], checkid) == 0) {
          fprintf(stderr, "graph disconnected \n");
          return 0;
        }
      }
    }
